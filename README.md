...
## Note sur `ReplyTemplate:`

Cette ligne est volontairement absente de l'en-tête. Winlink l'utilise pour désigner le modèle proposé en réponse au message reçu, et elle doit pointer vers un **autre** fichier `.txt` existant — typiquement un accusé de réception. Une auto-référence provoque l'erreur :
...
Si un modèle d'AR est ajouté plus tard dans le même dossier, rétablir la ligne en deuxième position :

```
ReplyTemplate: AR ADRASEC 06.txt
```
